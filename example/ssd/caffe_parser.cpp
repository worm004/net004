...
void CaffeModelParser::write_net(const std::string& net_path){
	...
	for(int i=0;i<layers.size();++i){
		...
		else if(layer_type == "Normalize") write_net_normalize(layer_name,param,ofile);
		else if(layer_type == "Permute") write_net_permute(layer_name,param,ofile);
		else if(layer_type == "Flatten") write_net_flatten(layer_name,param,ofile);
		else{
...
void CaffeModelParser::write_net_permute(const std::string& layer_name, const caffe::LayerParameter& param, std::ofstream& ofile){
	ofile<<"Layer: permute "<<layer_name<<endl;
	const caffe::PermuteParameter& permute_param = param.permute_param();
	ofile << permute_param.order()[0]<<" "<< permute_param.order()[1]<<" "<< permute_param.order()[2]<<" "<< permute_param.order()[3]<<endl;
}
void CaffeModelParser::write_net_flatten(const std::string& layer_name, const caffe::LayerParameter& param, std::ofstream& ofile){
	ofile<<"Layer: flatten "<<layer_name<<endl;
	const caffe::FlattenParameter& flatten_param = param.flatten_param();
	ofile<<flatten_param.axis()<<endl;
}
...
void CaffeModelParser::write_net_normalize(const std::string& layer_name, const caffe::LayerParameter& param, std::ofstream& ofile){
	const caffe::NormalizeParameter& normalize_param = param.norm_param();
	ofile<<"Layer: normalize "<<layer_name<<endl;
	ofile<<normalize_param.scale_filler().type()<<" "<<normalize_param.scale_filler().value()<<endl;
}
